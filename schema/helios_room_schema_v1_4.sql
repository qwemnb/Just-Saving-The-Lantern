...
-- =========================================================
CREATE TABLE room_history_visibility_events (
    id INTEGER PRIMARY KEY,
    room_id INTEGER NOT NULL,
    policy_version TEXT NOT NULL CHECK (policy_version = 'room_shared_v1'),
    effective_from_room_sequence_no INTEGER NOT NULL CHECK (effective_from_room_sequence_no = 1),
...
CREATE UNIQUE INDEX idx_room_history_visibility_events_room_sequence
    ON room_history_visibility_events(room_id, effective_from_room_sequence_no);
CREATE TRIGGER room_history_visibility_events_valid_insert
BEFORE INSERT ON room_history_visibility_events BEGIN
    SELECT CASE WHEN NEW.policy_version <> 'room_shared_v1'
        OR NEW.effective_from_room_sequence_no <> 1
        OR EXISTS (SELECT 1 FROM room_history_visibility_events WHERE room_id=NEW.room_id)
        OR EXISTS (SELECT 1 FROM messages WHERE room_id=NEW.room_id)
    THEN RAISE(ABORT, 'room history visibility event invalid') END;
END;
CREATE TRIGGER room_history_visibility_events_no_update
BEFORE UPDATE ON room_history_visibility_events BEGIN
    SELECT RAISE(ABORT, 'room history visibility events are immutable');
END;
CREATE TRIGGER room_history_visibility_events_no_delete
BEFORE DELETE ON room_history_visibility_events BEGIN
    SELECT RAISE(ABORT, 'room history visibility events are immutable');
END;
CREATE TRIGGER messages_visibility_guard
BEFORE INSERT ON messages BEGIN
    SELECT CASE WHEN NOT EXISTS (
        SELECT 1 FROM room_history_visibility_events
        WHERE room_id=NEW.room_id AND policy_version='room_shared_v1'
          AND effective_from_room_sequence_no=1
    ) THEN RAISE(ABORT, 'message_visibility_guard') END;
END;
...
VALUES
    ...
    (3, '1.4', 'Fresh room-wide canonical history visibility foundation.');
