...
This is SOW revision 3. It preserves the Participant Identity and Addressing
Foundation v1 milestone name and schema v1.3 target while superseding the prior
draft for implementation approval.
...
participants.name: Room
participant_type: system
primary alias: Room
bootstrap name event: exact bootstrap shape
```

Fresh initialization and migration must create the participant row, one
immutable `Room` alias row, the current-primary projection selecting that
alias, and one bootstrap name event using the exact bootstrap event shape
defined above.
...
Creating this identity during fresh initialization or migration must not create
a room membership, turn, routing row, API event, or visible canonical message.
...
10. Fail atomically if an existing participant bootstrap name normalizes to
    ...
    `room`.
11. Fail atomically if any v1.2 participant already has
    `participant_key = room-system`, even if its attributes appear to match
    the new identity. Schema v1.2 never establishes that reserved identity, so
    migration must not adopt, reinterpret, or repair a pre-existing row.
12. Only after the reserved-name and reserved-key checks pass, create the full
    room-owned bootstrap graph: `participant_key = room-system`,
    `participants.name = Room`, `participant_type = system`, one immutable
    `Room` alias, the current-primary projection selecting that alias, and one
    bootstrap name event using the exact bootstrap shape. Create no membership
    or visible canonical message.
13. Backfill exactly one immutable routing record for every existing canonical message.
14. Append schema migration 2 only after every v1.3 invariant is satisfied.
15. Run integrity, foreign-key, alias-cardinality, and message-routing-cardinality checks before commit.
16. Commit only if all checks pass.
...
- An existing v1.2 system message whose sender resolves to one existing
  participant and bootstrap alias is `legacy_implicit` to `room`, regardless
  of that historical sender's participant type or key.
...
Python's SQLite connection interface can reliably report only whether a
transaction is active; it cannot reliably prove that an existing transaction
began as `IMMEDIATE`. The service must verify that a transaction is active and
otherwise fail before writing. The caller remains contractually responsible
for using `BEGIN IMMEDIATE`. Integration tests with a second connection must
prove that the caller-owned transaction acquires the expected write lock before
the service performs conflict checks or writes.
...
3. For a structurally valid, nonblank request, run the shared local-command
   classifier before opening a database connection or loading provider state.
   Exact or malformed reserved `/trace` and `/participants` forms return the
   exact `local_command_only` contract defined below with zero side effects.
4. Resolve and authorize the structured destination inside the Phase A
   `BEGIN IMMEDIATE` transaction, before canonical acceptance.
5. Only for an authorized Helios destination, load and validate provider
   environment, select or create the semantically exact configuration, run
   memory retrieval, and record the provider request.
6. For a Room destination, never load provider environment, select or create a
   provider configuration, search memory, construct a provider client, or
   create an API event.

Because exact request-shape validation has precedence, a structurally invalid
body always returns `invalid_message_request`, even when a supplied
`message_text` value resembles a reserved local command. The
`local_command_only` contract applies only after the complete request has
passed structural validation and whitespace-only handling.
...
Revise `store-message` as a Room-only canonical note writer. It must require
...
The command must reject `--destination-kind participant`, every
`--participant-key` argument, a missing destination, and every unsupported or
ambiguous combination before opening a write transaction. It must never invoke
Helios, load provider environment, select or create provider configuration,
search memory, construct a provider client, or create an API event.

For an accepted Room note, it must apply the same Room authorization, shared
local-command interception, exact-message preservation, and atomic
message-plus-route rules as the HTTP Room path. It creates one Peter-initiated
turn, one Peter chat message, one explicit Room route, and marks the turn
completed in the same `BEGIN IMMEDIATE` transaction. It must never leave an
open CLI-created turn. The machine-readable result may contain canonical IDs
and stable status fields but must not echo unrestricted message text or
secrets.
...
Valid legacy and explicit system history is omitted from Helios provider input.
It must never cause a later valid Helios turn to fail merely because its message
type or historical sender is system-owned.
...
- supported Helios chat responses directed to Peter are included as
  `assistant` items
- valid `legacy_implicit` v1.2 system messages routed to Room are omitted after
  their sender, bootstrap sender-alias snapshot, Room destination-alias
  snapshot, and migration-valid message shape are verified; omission does not
  depend on the historical sender's participant key or type
- new `explicit` system messages are omitted only when they are authored by
  `room-system`, routed to Room with valid immutable alias snapshots, and
  correlated one-to-one with a valid `adopted` participant-name event
- every other system message, correction or retraction message, unsupported
...
If any part fails, roll back the entire transaction and return the exact
`room_post_failed` contract defined below without reflecting unrestricted
message text or an internal exception.
...
Return the exact `participant_destination_unavailable` contract defined below.
Do not echo the submitted key in the response.
...
Structurally valid exact or malformed reserved local commands return exactly:

```text
HTTP 400
error: local_command_only
message: Local commands are available only in the browser interface.
Cache-Control: no-store
```

An unavailable participant destination returns exactly:

```text
HTTP 409
error: participant_destination_unavailable
message: The selected participant destination is unavailable.
Cache-Control: no-store
```

An internal failure while atomically saving a Room post returns exactly:

```text
HTTP 500
error: room_post_failed
message: The Room message could not be saved.
Cache-Control: no-store
```

These three errors must not expose submitted participant keys, unrestricted
message text, exception text, validation internals, SQL, paths, credentials,
provider settings, memory controls, or secrets in response bodies or headers.

The `GET /api/participants` and `GET /api/messages` routes must also return
`Cache-Control: no-store` on success and failure.

Participant-directory data inconsistency returns exactly:
...
An unavailable or incompatible participant-directory database returns exactly:

```text
HTTP 503
error: participant_directory_unavailable
message: The participant directory is unavailable.
Cache-Control: no-store
```

Message-history data inconsistency returns exactly:
...
An unavailable or incompatible message-history database returns exactly:

```text
HTTP 503
error: message_history_unavailable
message: The message history is unavailable.
Cache-Control: no-store
```

These responses must not expose unrestricted exception text, SQL, paths,
aliases from a rejected row, credentials, configurations, private memory, or
other rejected data.

Add real-ASGI tests with distinctive secret sentinels in an unknown top-level
field, an unknown nested destination field, an unavailable participant key, a
malformed local-command argument, and Room message text during injected
failure. Assert every exact status, code, message, `no-store` header, and the
absence of rejected names and values from response bodies and headers.
...
- Fresh initialization and migration create the exact `room-system` participant
  row, immutable `Room` alias, current-primary projection, and bootstrap name
  event without a membership or canonical message.
- Migration rejects a normalized alias collision atomically.
- Migration rejects reserved bootstrap names and every pre-existing v1.2
  `room-system` participant, including an apparently matching row.
...
- The adoption service rejects a connection with no active transaction, while
  a two-connection integration test proves the caller's `BEGIN IMMEDIATE`
  acquires the write lock; the service is not required to infer transaction
  mode from Python's SQLite connection object.
...
- Invalid directory data returns the exact `participant_directory_invalid`
  contract without leaking rejected data.
- An unavailable or incompatible directory database returns the exact
  `participant_directory_unavailable` contract.
...
- A structurally invalid request returns `invalid_message_request` even when
  its `message_text` resembles `/trace` or `/participants`.
...
- Reserved local commands return the exact `local_command_only` HTTP contract.
- Unavailable participant destinations return the exact
  `participant_destination_unavailable` HTTP contract.
...
- Exact and malformed `/participants` forms in structurally valid requests are
  blocked through direct API and CLI entry points with `local_command_only` and
  zero side effects.
- `store-message` accepts only an explicit Room destination, creates a
  completed turn plus message and route atomically, and never leaves an open
  turn.
- `store-message` rejects participant destinations and every participant-key
  argument before opening a write transaction.
...
- A valid CLI Room note has the same completed-turn and zero-provider
  guarantees as an HTTP Room post.
...
- An injected internal Room-post failure returns the exact `room_post_failed`
  contract without message-text, exception, path, SQL, or secret leakage.
- Room message history and trace contain exact text and correct routing.
- Invalid message-history data returns the exact `message_history_invalid`
  contract without leaking rejected data.
- An unavailable or incompatible history database returns the exact
  `message_history_unavailable` contract.
- A Room-directed Peter message appears as an ordinary `user` history item in
  a later Helios provider request.
- A migrated, valid `legacy_implicit` v1.2 system message routed to Room is
  omitted from a later Helios provider request regardless of its historical
  sender and does not break the turn.
- Valid room-system name-adoption messages are omitted from provider input
  without breaking later turns.
- An `explicit` system message lacking `room-system` authorship, a Room route,
  valid alias snapshots, or a one-to-one adopted-name event fails closed.
- Every other unsupported system, correction, retraction, participant, or
  routing history fails closed.
...
- the `room-system` identity has bootstrap name `Room`, primary alias `Room`,
  one bootstrap event, no membership, and no creation-time canonical message
...
- reserved-command, unavailable-destination, and Room-post internal failures
  use their exact no-store HTTP contracts without rejected-data leakage
- Room-directed chat remains later Helios-visible history while valid
  migrated legacy system history and valid room-system adoption events are
  omitted
- the adoption service uses caller-owned transaction boundaries exclusively
- `store-message` is Room-only, completes its turn atomically, and cannot
  create an unrouted, implicitly routed, participant-directed, or open-turn
  message
