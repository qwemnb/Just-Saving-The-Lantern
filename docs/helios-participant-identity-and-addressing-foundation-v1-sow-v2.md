...
This is SOW revision 2. It preserves the Participant Identity and Addressing
...
12. Create a new room-owned system identity and its `Room` alias only after
    the reserved-name and reserved-key checks pass.
...
3. For a structurally valid, nonblank request, run the shared local-command
   ...
   stable generic `local_command_only` contract with zero side effects.
