...
This is SOW revision 4 for Gemini Participant Integration v1.
...
Construct `GenerateContentConfig` with exactly this SDK-local automatic-
...
the one explicit `models.generate_content` provider call. It is SDK-local
orchestration policy, not a Gemini API request member. The pinned SDK's API
converter does not emit it on the wire. Record it under the immutable
configuration's `sdk_policy` and the request event's
`local_context.sdk_policy`; do not place it under the recorded transmitted
...
provider defaults apply. The transport timeout, attempt count, API version,
safety marker, and SDK automatic-function-calling policy belong to recorded
local context; they are not invented members of the transmitted Gemini API
request body.
...
- SDK-local automatic function calling disabled under
  `local_context.sdk_policy` and absent from the transmitted request shape;
- candidate count;
- max output tokens;
- response modalities;
...
Offline SDK-boundary tests must inspect both representations: prove that
`AutomaticFunctionCallingConfig(disable=True)` reaches the pinned SDK's local
configuration, and capture the request produced by the pinned SDK API converter
to prove that no `automatic_function_calling` member appears anywhere in the
wire JSON. The recorded request event must match that separation exactly.
...
function calling is disabled in the SDK-local configuration, so this member
...
`^[A-Za-z0-9_.:-]{1,256}$`. `error_class` is always the fixed local literal
`ProviderError` for a non-timeout provider exception and `TimeoutError` for a
timeout. It must never contain an SDK, transport, Python, or attacker-controlled
exception class name. `reason` and `summary` are likewise the fixed literals
...
Its `error_class` is the fixed local literal shown. Together,
`ProviderError`, `TimeoutError`, and `ResponseSerializationError` are the
closed `error_class` enum for this milestone. It permits no optional fields and
stores no partial response, rejected key/value, exception text, or provider
metadata.
...
The extensible raw-response policy is a database-evidence policy, not a browser
extension policy. The Gemini Trace projector must use a closed set of fields
defined by this SOW: requested/resolved model, candidate finish status, bounded
safety status, the five recognized scalar usage counts, accepted visible
output or fixed safe error summary, event correlation, and the separately
authorized inherited-memory audit. Any response member outside the recognized
semantic core or those named Trace fields—including any unknown future top-
level or nested provider field—must be omitted from the browser projection by
default. For every such omission, add the exact RFC 6901 JSON pointer to the
existing omitted-pointer collection, rooted at the parsed event payload (for
example, `/response/future_field`). Escape `~` as `~0` and `/` as `~1`, and
point to the highest omitted extension subtree so no descendant value is
visited or exposed. Omission does not invalidate otherwise bounded raw database
evidence.
...
`x-goog-api-key`. Secret and reasoning omissions take precedence over the
generic extension omission rule but use the same deduplicated, lexicographically
sorted RFC 6901 pointer collection. Provider-extension values must never be
recursively projected to the browser and never bypass recognized semantic
checks.
...
- automatic function calling is explicitly disabled in the SDK configuration,
  recorded `local_context.sdk_policy`, and immutable configuration, with no
  AFC-generated call or history;
- an injected pinned-SDK converter/transport capture proves the local AFC
  setting is applied but `automatic_function_calling` is absent from wire JSON
  and recorded `request.config`;
...
- provider subclasses and attacker-named exception types cannot change the
  closed local `error_class` literals `ProviderError`, `TimeoutError`, and
...
- unknown top-level and nested response extensions remain in bounded raw
  database evidence but are absent from browser Trace, with exact escaped RFC
  6901 pointers to each highest omitted subtree;
- extension values containing HTML, credentials, aliases, private reasoning,
  and attacker-controlled error text never reach Trace output or the DOM;
