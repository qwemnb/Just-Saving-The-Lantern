...
- recipient alias ID used for display when the message was created
...
- a participant destination has both a recipient participant and a recipient alias belonging to it
- a room destination has neither recipient field
...
10. Create the room-owned system identity and its `Room` alias.
11. Backfill exactly one immutable routing record for every existing canonical message.
12. Append schema migration 2 only after every v1.3 invariant is satisfied.
13. Run integrity, foreign-key, alias-cardinality, and message-routing-cardinality checks before commit.
14. Commit only if all checks pass.
...
Fresh database initialization must produce the same v1.3 graph without first installing v1.2 and then mutating it.
...
4. Begin or require an existing `BEGIN IMMEDIATE` transaction.
...
14. Commit all changes atomically through the caller's transaction.
...
### 4. `/participants` browser command

Recognize exact `/participants` with optional surrounding whitespace as a browser-only command. It refreshes and opens the participant panel or drawer.
...
- Existing system messages route to Room.
...
- Extra top-level and nested keys are rejected.
...
- invalid destinations fail before canonical acceptance and provider construction
