    ...
    def test_helios_source_invokes_gemini_once_and_adds_one_message(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "HELIOS_GEMINI_MODEL": "gemini-2.5-flash-lite",
            },
            clear=True,
        ):
        ...
        thinking = gemini_client.aio.models.calls[0]["config"].thinking_config
        self.assertEqual(thinking.thinking_budget, 0)
        self.assertIsNone(thinking.thinking_level)
        self.assertIs(thinking.include_thoughts, False)
        ...
        with closing(connect_database(self.database)) as connection:
            ...
            self.assertEqual(
                payload["request"]["config"]["thinking_config"],
